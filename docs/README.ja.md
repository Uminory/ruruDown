...
> エクスポート先のディレクトリを`Download`以外に変更することを推奨いたします。なぜなら、一部のシステムバージョンでは、`Download`ディレクトリ内のメディアファイルを削除してもストレージ容量が解放されない問題があるためです。
> これはruruDownの問題ではなく、システム自体の欠陥によるものです。占有された容量は**デバイスを再起動する**ことで解放されます。
